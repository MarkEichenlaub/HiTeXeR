...
// Prefer the INTRINSIC dimensions (pre-container-fit): each block must
// compose at its natural size, the same way the rasterizer treats single
// diagrams — the width="" attribute is capped to the render container
// (800px), which shrank large blocks and left the whole stacked document
// ~30% smaller than TeXeR's (12948: 481bp wide vs ref 661bp).
function parseSvg(svg) {
  const open = svg.match(/<svg\b[^>]*>/i);
  if (!open) return null;
  const tag = open[0];
  const w = parseFloat((tag.match(/data-intrinsic-w="([\d.]+)"/) || tag.match(/\bwidth="([\d.]+)"/) || [])[1] || '0');
  const h = parseFloat((tag.match(/data-intrinsic-h="([\d.]+)"/) || tag.match(/\bheight="([\d.]+)"/) || [])[1] || '0');
