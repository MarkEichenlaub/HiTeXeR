...
function buildPrompt(args) {
  ...
  return [
    ...
    '- The bar for "good"/"minor" is: would a math student say the TEST diagram is',
    '  THE SAME mathematical figure as the REFERENCE? If any element carries',
    '  mathematical meaning and is wrong — a label reads a different number/letter,',
    '  an angle/arc is on the wrong vertex, a point sits at the wrong coordinate, a',
    '  curve bends the wrong way, a region is shaded that should not be — that is',
    '  "poor", even if it is a small part of the picture. Do NOT average it away',
    '  against the parts that match. When uncertain between "minor" and "poor",',
    '  choose "poor": this rating gates an automated code change, so a near-miss',
    '  that is actually wrong is worse than a false alarm.',
    '',
    'Begin.',
