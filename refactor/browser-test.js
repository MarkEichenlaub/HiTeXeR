...
(async()=>{
 ...
 const t0=Date.now();
 await p.goto(process.env.HTX_URL||'http://localhost:8765/index.html',{waitUntil:'load',timeout:60000});
 ...
 const res=await p.evaluate(async(ids)=>{const out=[];
  for(const id of ids){const src=await (await fetch((window.__srcBase||'')+'comparison/asy_src/'+id+'.asy')).text();const code='[asy]\n'+src+'\n[/asy]';
