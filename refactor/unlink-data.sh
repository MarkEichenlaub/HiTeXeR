...
for d in node_modules comparison/texer_pngs comparison/asy_src refactor/conformance/.cache; do
  [ -e "$d" ] || continue
  win=$(cygpath -w "$d")
  if powershell -NoProfile -Command "if ((Get-Item -Force '$win').LinkType -in @('Junction','SymbolicLink')) { exit 0 } else { exit 1 }"; then
