  ...
  for (const {ci, dc, css} of deferredLabels) {
    if (dc.cmd === 'label') {
      ...
      if (/\\definecolor/.test(displayText)) {
        const colorDefs = {};
        displayText = displayText.replace(/\\definecolor\s*\{([^}]*)\}\s*\{([^}]*)\}\s*\{([^}]*)\}/g, (_m, name, model, values) => {
          if (model.toUpperCase() === 'RGB') {
            const parts = values.split(',').map(v => parseInt(v.trim(), 10));
            if (parts.length === 3) {
              colorDefs[name] = '#' + parts.map(c => Math.max(0, Math.min(255, c)).toString(16).padStart(2, '0')).join('');
            }
          }
          return '';
        });
